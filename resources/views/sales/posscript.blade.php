<script>
    $(document).ready(function() {
        var qrcodetest = $('#invoice_code').val()
        ...
        function PrintPayment() {
            ...
            var getscr = $('#qrcode-2').find('img').attr('src')
     ...
     `
        ...
        `
            ...
            var invoiceno = $('#invoice_code').val()
            ...
            newWin.document.write(`
                        ...
                        <div class="printimg">
                            <img src="{{ asset('img/blue1.webp') }}" alt=""></br>
                            
                            </div>
                        
                        <div class='print-title'> <h1>Invoice  </h1>Invoice Code:${invoiceno+1}</div>
                        <hr>
              ...
              <footer>
                <div class="result">Hello Wolrld</div>
                <footer>
        </body>
    ...
    `)
                ...
                if (letable >= 2) {
                    if (payby == "") {
                        ...
                        for (var i = 1; i < letable; i++) {
                            var itemno = $('.table').find(`tr:eq(${i}) td:eq(0)`).text()
            ...
            $('body').on('change', '#discount_type', function() {
                ...
                if (discount_typ == 1) {
                    for (var i = 1; i < getertotal; i++) {
                        ...
                        $('.table').find(`tr:eq(${Number(i)}) td:eq(3)`).text((Number(price) * Number(
                            qty) - Number(price) * Number(qty) * (discount) / 100).toFixed(4));
                    }
                } else {
                    ...
                    for (var i = 1; i < getertotal; i++) {
                        ...
                        discount = $('.table').find(`tr:eq(${Number(i)}) th:eq(0) input`).val()
                        $('.table').find(`tr:eq(${Number(i)}) td:eq(3)`).text((Number(price) * Number(
                            qty) - Number(qty) * (discount)).toFixed(4));
                    }
            ...
            table.html(`<thead >
                             <tr  >
                                ...
                                <th>Item_no </th>
                                 <th>Price </th>
                                 <th>Qty   </th>
                                 <th>Discount <select name="discount_type" id="discount_type"  style="border:1px solid skyblue;width:60px;border-radius:2px">
        ...
        $('.todaysales').on("click", function() {
            $('.todaysale').css({
                'display': 'block'
            })
        })
        $('.returnsales').on('click', function() {
            $('.return_sales').css({
                'top': '50%'
            });
            $('.back_return').css({
        ...
        function Return_Product() {
            $('.returns').on("click", function() {
                ...
                var invoice = $('#return_invoice_input').val()
                var td = '';
                $('.return_invoice').html(`
             <thead>
             <th>Select</th>
                        ...
                        success: function(data) {
                            if (data.status.length >= 1) {
                                data.status.forEach(element => {
                                    ...
                                    td += `
                        <tr>
                        <td><input type="checkbox" name="" id="checkproduct"></td>
                <td>${element.item_no}</td>
                <td>${Number(element.unit_price).toFixed(2)}</td>
                <td>${Number(element.quantity).toFixed(2)}</td>
                <td>${Number(element.discount_amount).toFixed(2)}</td>
                <td>${element.unit_of_measure}</td>
                <td>${Number(element.amount).toFixed(2)}</td>
                </tr>
                        `
                                });

                                $('.return_invoice').append(td)
                            } else {
                                alert("Not Found")
                            }
        ...
        $('.cansel_return').on('click', function() {
            $('.back_return').css({
                'display': 'none'
            });
            $('#return_invoice_input').val("");
            $('.return_check').text('00')
            $('.return_sales').css({
                'top': '-900'
            });
            $('.return_invoice').html(`
        ...
        $('body').on('click', '#checkproduct', function() {
            ...
            var par = ethis.parents('tr');
            var index = par.index();
            var checkadd = $(this).prop('checked')

            var price = par.find('td:eq(6)').text()
            var totalprice = $('.return_check').text()
            if (checkadd == true) {
                totalprice = Number(totalprice) + Number(price)
            } else {
                totalprice = Number(totalprice) - Number(price)
            }
            $('.return_check').text(Number(totalprice).toFixed(2))
        })
        $('.submit_return').on('click', function() {
            var table_length = $('.return_invoice').find('tr').length

            for (let i = 1; i < table_length; i++) {
                var check = $('.return_invoice').find(`tr:eq(${i}) td:eq(0) input`).prop('checked')
                var invoice = $('#return_invoice_input').val();

                if (check == true) {
                    var item_no = $('.return_invoice').find(`tr:eq(${i}) td:eq(1)`).text()
                    $.ajax({
                        url: 'SubmitReturnProduct',
                        type: 'POST',
                        data: {
                            'invoice': invoice,
                            'item_no': item_no,
                ...
                if (check == true) {
                    $.ajax({
                        ...
                        success: function(data) {
                            var customername = $('#customer').find('option:selected').text()
                            var printtable = $('.return_invoice').find('tr').length
                            var id = $('.table').find(`tr:eq(${Number(i)}) td:eq(3)`)
                        .text();
                            var thead = `
                                <table class="tablee">
                                <thead  class="thead-dark">
     ...
     `
         ...
         <tr >
                       ...
                       <td>${$('.return_invoice').find(`tr:eq(${Number(i)}) td:eq(1)`).text()}</td>
                       <td> ${$('.return_invoice').find(`tr:eq(${Number(i)}) td:eq(2) `).text()}</td>
                       <td> ${$('.return_invoice').find(`tr:eq(${Number(i)}) td:eq(3) `).text()}</td>
                       <td> ${$('.return_invoice').find(`tr:eq(${Number(i)}) td:eq(4) `).text()}</td>
                       <td>${$('.return_invoice').find(`tr:eq(${Number(i)}) td:eq(5)`).text()}</td>
                       <td>${$('.return_invoice').find(`tr:eq(${Number(i)}) td:eq(6)`).text()}</td>
        ...
        `
                            ...
                            var totalprice = $('.totalprice').find('span').text();
                            var invoiceno = $('#invoice_code').val()
                            var newWin = window.frames["printfreturn"];
                            newWin.document.write(`
